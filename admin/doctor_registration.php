  ...
  if(isset($_POST['submit'])){
    ...
    if($check > 0){
      echo "<script>alert('You have already registered')</script>";
        ...
        $ins1 = mysqli_query($conn , "INSERT INTO `user_doctor`(`unique_id`, `fname`, `lname`, `email`, `password`, `img`, `status`,`i_am`) VALUES('$z','$first_name','$last_name','$email','$password','$image_data','Offline now','doctor')");
          $ins = mysqli_query($conn , "INSERT INTO `doctor_registration`(`first_name`, `last_name`, `email`, `phone`, `date`, `reg_no`, `gender`, `year_of_exp`, `city`, `address`, `password`, `image`) VALUES('$first_name','$last_name','$email','$phone','$date','$reg_no',' $gender','$year_of_exp','$city','$address','$pass','$image_data')");
