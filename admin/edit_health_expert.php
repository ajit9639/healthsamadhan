<?php include "header.php";
   $get = $_GET['id'];
    ...
    if($type == "delete"){
    $del = mysqli_query($conn , "DELETE FROM `health_expert_registration` WHERE `id`='$get'");
    echo "<script>alert('Doctor Deleted Successfully')</script>";
    echo "<script>window.location.href='dietitian_view.php';</script>";
    }else{
 
    $mysqrives = mysqli_fetch_assoc(mysqli_query($conn , "SELECT * FROM `health_expert_registration`  WHERE `id`=' $get'"));
  ...
  if(isset($_POST['submit'])){
    ...
    // }else{
    $ins = mysqli_query($conn , "UPDATE `health_expert_registration` SET `first_name`='$first_name',`last_name`='$last_name',`email`='$email',`phone`='$phone',`date`='$date',`reg_no`='$reg_no',`gender`='$gender',`year_of_exp`='$year_of_exp',`city`='$city',`address`='$address' where `id`='$get'");
    if($ins){
        echo"<script>
        ...
        <div class="content-wrapper">
            ...
            <div class="row">
                <div class="col-12 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">Edit Health Expert</h4>
